...
int SPRPacket::serialize(char* buffer, int64_t len, int64_t& pos)
{
	...
	{
		Log(LOG_ERR,"SPRPacket", "invalid argument buffer=%p, len=%ld, pos=%ld",
			buffer, len, pos);
	...
	else
	{
		...
		{
			Log(LOG_ERR, "SPRPacket", "serialize status failed, buffer=%p, len=%ld, status=%u, pos=%ld, ret=%d",
				buffer, len, status_, pos, ret);
		}
		else if (SUCCESS != (ret = Util::store_int4(buffer, len, statement_id_, pos)))
		{
			Log(LOG_ERR, "SPRPacket", "serialize warning_count failed, buffer=%p, len=%ld, statement_id_=%u, pos=%ld, ret=%d",
				buffer, len, statement_id_, pos, ret);
		}
		else if (SUCCESS != (ret = Util::store_int2(buffer, len, column_num_, pos)))
		{
			Log(LOG_ERR, "SPRPacket", "serialize marker failed, buffer=%p, len=%ld, column_num_=%u, pos=%ld, ret=%d",
				buffer, len, column_num_, pos, ret);
		}
		else if (SUCCESS != (ret = Util::store_int2(buffer, len, param_num_, pos)))
		{
			Log(LOG_ERR, "SPRPacket", "serialize marker failed, buffer=%p, len=%ld, param_num_=%u, pos=%ld, ret=%d",
				buffer, len, param_num_, pos, ret);
		}
		else if (SUCCESS != (ret = Util::store_int1(buffer, len, reserved_, pos)))
		{
			Log(LOG_ERR, "SPRPacket", "serialize marker failed, buffer=%p, len=%ld, reserved_=%u, pos=%ld, ret=%d",
				buffer, len, reserved_, pos, ret);
		}
		else if (SUCCESS != (ret = Util::store_int2(buffer, len, warning_count_, pos)))
		{
			Log(LOG_ERR, "SPRPacket", "serialize marker failed, buffer=%p, len=%ld, warning_count_=%u, pos=%ld, ret=%d",
