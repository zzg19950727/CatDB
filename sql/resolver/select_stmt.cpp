...
u32 OrderStmt::deep_copy(OrderStmt_s &order, u32 flag)const
{
	u32 ret = SUCCESS;
	ExprStmt_s copy_expr;
	CHECK(ExprUtils::deep_copy_expr(order_expr, copy_expr, flag));
...
u32 SelectStmt::deep_copy(SelectStmt_s &stmt, u32 flag)const
{
	...
	for (u32 i = 0; i < order_exprs.size(); ++i) {
		CHECK(order_exprs[i]->deep_copy(copy_order, flag));
